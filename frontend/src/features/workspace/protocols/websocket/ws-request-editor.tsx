...
import { WsOptionsEditor } from "./ws-options-editor";

type Props = {
  tab: RequestTab;
};

export function WsRequestEditor({ tab }: Props) {
  ...
  return (
    <section className="flex min-h-0 h-full flex-1 flex-col bg-[hsl(var(--editor))]">
      <Tabs defaultValue="headers" className="flex min-h-0 flex-1 flex-col">
        ...
        <TabsContent value="websocket" className="mt-0 min-h-0 flex-1 overflow-auto">
          <WsOptionsEditor tab={tab} />
        </TabsContent>
      </Tabs>
    </section>
  );
