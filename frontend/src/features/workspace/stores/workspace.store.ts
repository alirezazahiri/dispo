...
function sanitizeState(
  ...
  const currentCollectionId =
    ...
    collectionIds.includes(state.currentCollectionId)
      ? state.currentCollectionId
      : (collectionIds[0] ?? "");
...
function serializeTabs(state: WorkspaceStore & WorkspaceUiState): RequestTab[] {
  return Object.values(state.tabsById).sort(
...
function getFallbackCollectionId(
  ...
  const fromTabs = Object.keys(tabOrderByCollection).find((id) =>
...
export const useWorkspaceStore = create<WorkspaceStore & WorkspaceUiState>()(
  (set, get) => ({
    ...
    createTab: (protocol = "http", collectionId) => {
      ...
      set((state) => ({
        ...
        tabOrderByCollection: {
          ...state.tabOrderByCollection,
          [targetCollectionId]: [
            ...(state.tabOrderByCollection[targetCollectionId] ?? []),
            tab.id,
          ],
        },
        activeTabIdByCollection: {
    ...
    setCurrentCollection: (collectionId) => {
      const current = get();
      const hasTabs =
        (current.tabOrderByCollection[collectionId] ?? []).length > 0;
    ...
    saveTabToCollection: async (tabId, options) => {
      ...
      const existingRequest = tab.savedRequestId
        ? (Object.values(requestsByCollection)
            .map((requests) => requests[tab.savedRequestId as string])
            .find(Boolean) ?? null)
        : null;

      const targetCollectionId =
        options?.collectionId ??
        existingRequest?.collectionId ??
        tab.collectionId;
      const folderIdExplicitlyProvided =
        options !== undefined && "folderId" in options;
      const targetFolderId = folderIdExplicitlyProvided
        ? (options!.folderId ?? null)
        : (existingRequest?.folderId ?? null);
    ...
    removeCollectionState: (collectionId) => {
      set((state) => {
        ...
        const tabIdsToRemove = new Set(
          state.tabOrderByCollection[collectionId] ?? [],
        );
        ...
        for (const [id, tabIds] of Object.entries(nextTabOrderByCollection)) {
          nextTabOrderByCollection[id] = tabIds.filter(
        ...
        const nextCurrentCollectionId = remainingCollectionIds.includes(
          state.currentCollectionId,
        )
          ? state.currentCollectionId
    ...
    reconcileCollections: (collectionIds) => {
      set((state) => {
        ...
        for (const [collectionId, tabIds] of Object.entries(
          state.tabOrderByCollection,
        )) {
          if (!valid.has(collectionId)) {
            continue;
          }
          const filtered = tabIds.filter((tabId) =>
            Boolean(nextTabsById[tabId]),
          );
          if (filtered.length > 0) {
        ...
        for (const [collectionId, tabId] of Object.entries(
          state.activeTabIdByCollection,
        )) {
          if (!valid.has(collectionId)) {
            continue;
